...
void DrawRectOutline(BYTE* Img, int W, int H, int LU_X, int LU_Y, int RD_X, int RD_Y, int bitcount)
{
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if (bitcount == 24) { //true color
...
void BinaryImageEdgeDetection(BYTE* Bin, BYTE* Out, int W, int H,int bitcount)
{
	...
	DrawRectOutline(Out, W, H, min_x, max_y, max_x, min_y,bitcount);
}
...
int main()
{
	...
	*/
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if (Cb[i * W + j] > 95 && Cb[i * W + j] < 123 && Cr[i * W + j] > 130 && Cr[i * W + j] < 160) {
				Output[i * W * 3 + j * 3] = Image[i * W * 3 + j * 3];
				Output[i * W * 3 + j * 3 + 1] = Image[i * W * 3 + j * 3 + 1];
				Output[i * W * 3 + j * 3 + 2] = Image[i * W * 3 + j * 3 + 2];
	...
	//이전의 했던 과제를 참고하여 함수를 약간 변형하여 사용하였습니다.
	DrawRectOutline(Output, W, H, min_x, max_y, max_x, min_y, hInfo.biBitCount);
